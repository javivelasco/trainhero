...
s.linkInternalFilters="javascript:,renfe.es,renfe.com"
...
function s_doPlugins(s) {
	...
	if(s.eVar10) s.events=s.apl(s.events,'event4',',',0);
	
	/* Copy of variables */
	if(s.purchaseID&&!s.eVar5) s.eVar5 = s.purchaseID;
	if(s.prop9&&!s.eVar7) s.eVar7 = "D=c9";
	...
	if(s.prop4){
		s.eVar4="D=c4";
		s.events=s.apl(s.events,'event5',',',0);
	}
	
	/*Time parting*/
	s.prop7 = s.getTimeParting('h','+1',new Date().getFullYear())+'-'+s.getTimeParting('d','+1',new Date().getFullYear());
	s.eVar28 = "D=c7"; 
...
s.visitorNamespace="renfe"
s.trackingServer="renfe.d3.sc.omtrdc.net"
